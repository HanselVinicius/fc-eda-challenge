...
func MigrateAndSeed(db *sql.DB) {
	...
	var exists int
	db.QueryRow("SELECT COUNT(*) FROM clients").Scan(&exists)
	if exists == 0 {
		fmt.Println("Seed: Populando dados iniciais...")
		db.Exec("INSERT INTO clients VALUES ('1', 'Client 1', 'c1@email.com', NOW())")
		db.Exec("INSERT INTO clients VALUES ('2', 'Client 2', 'c2@email.com', NOW())")
		db.Exec("INSERT INTO accounts VALUES ('acc1', '1', 1000, NOW())")
		db.Exec("INSERT INTO accounts VALUES ('acc2', '2', 2000, NOW())")
		db.Exec("INSERT INTO transactions VALUES ('t1', 'acc1', 'acc2', 100, NOW())")
	}
}
